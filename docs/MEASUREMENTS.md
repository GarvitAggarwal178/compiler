...
| j2-parse-coverage | 2026-08-22 | `python3 harness/parse_coverage.py` | `tests/corpus/IN_GRAMMAR.txt` (195 files) | n/a | `195/195 not_implemented`, exit 0 | M1 day-2 acceptance gate, exists before the parser does. `docs/reports/J2-m1-gates.md`. |
| j2-round-trip | 2026-08-22 | `python3 harness/round_trip_scaffold.py` | same 195 files | n/a | `195/195 not_implemented`, exit 0 | parse→print→reparse→compare harness; printer is a Lane A stub. `docs/reports/J2-m1-gates.md`. |
| j2-rejection-verify | 2026-08-22 | `python3 harness/run_rejection_tests.py` | 13 rejection cases, 4 grounds | n/a | `13/13 not_implemented` | +1 allowedness case (`allowedness_equation_rhs_not_bound`) added from J1 case (d), independently re-verified against Soufflé 2.5 (`Ungrounded variable y`). `docs/reports/J2-m1-gates.md`. |
| night02-t1-allowedness-probe-{h..o} | 2026-08-23 | `souffle -F measurements/_scratch_night02_t1/facts -D ... tests/programs/allowedness_probe_{h..o}.dl` | 8 probe programs | n/a | accept: i,k,l,n,o; reject: h,j,m (`Ungrounded variable {Y,X,Y}`) | h is the load-bearing case: body-only variable in a constraint (not in head) still rejects -- allowedness quantifies over more than head variables. No definition proposed. Full table `docs/reports/night02-T1-allowedness.md`. |
| night02-t3-fixtures | 2026-08-23 | `python3 harness/night02_t3_fixtures.py` | 4 shapes x their scale points (38 `.facts` files) | n/a | idempotent: true (38/38 SHA-256 identical across 2 runs); `transitive_closure_bound`/`ancestor_nonancestor` reachable-from-1 = 50 at every n (matches `core_size`); `same_generation_negation` reachable-from-0 = `n_persons` at every depth (whole tree, root) | Fixture generation only, no Soufflé run. One bug caught before trusting numbers: initial same-generation reachability walk used the wrong edge direction (root has no outgoing child->par edge), fixed and re-verified. Full tables `docs/reports/night02-T3-fixtures.md`. |
| night02-t4-baseline | 2026-08-23 | `python3 harness/night02_t4_baseline.py` (souffle, untransformed + `--magic-transform=*`, per shape per scale point) | 5 shapes, 24 scale points, 48 runs | off vs on | no DNF, no abort, all 24 points answers-identical; `transitive_closure_bound` E_recoverable=0 at every point (positive fragment, disclosed as a null result); `reachability_complement` numbers match T6 exactly at all 6 points | excl-copy/incl-copy never diverged in this sweep (no COPY_T relation in any of 48 profiles) -- reported as a null result. Full per-shape tables `docs/reports/night02-T4-baseline.md`. |
| night02-t5-guarded | 2026-08-23 | `python3 harness/night02_t5_guarded.py` (hand-guarded `.dl` under `tests/corpus/BENCHMARK_FAMILY/guarded/`) | 4 shapes (excl. `transitive_closure_bound`, E_recoverable=0), 20 scale points | `T_souffle/T_guard` | no DNF, no abort, all 20 points answer-identical to T4 baseline; ratios: same_generation_negation 68x-4,371x; ancestor_nonancestor 4x-62x; culprit_cycle ~1.0x (no contribution); reachability_complement 157x-4,244x (matches T6 exactly) | culprit_cycle's general-adornment derivation was rejected by Soufflé (cyclic negation, `Unable to stratify {m_q,p_bf,q_bf,s_bf}`) -- discarded per requirement, not fixed up; safe partial-restriction guard used instead. Also found: Soufflé can exit 0 on a stratification error. Full per-shape tables and explanations `docs/reports/night02-T5-guarded.md`. |
| night02-t6-scaling-crossshape | 2026-08-23 | pairwise log-log slope over already-committed T3/T4/T5 numbers, no new Soufflé runs | 5 shapes | growth class per shape | 3/4 guarded shapes (same_generation_negation, ancestor_nonancestor, reachability_complement) all Θ(n)/Θ(n²) growth class for T_guard/T_none, hence ratio Θ(n) -- but magnitudes differ by up to 2 orders of magnitude at comparable n; culprit_cycle disagrees on class (flat ~1.0x ratio, no growth); transitive_closure_bound T_souffle flat Θ(1) (no guard needed) | culprit_cycle's 5 points do not admit a clean power-law fit (slopes 0.5-2.7 pair to pair) -- reported as noisy, not smoothed. Full tables `docs/reports/night02-T6-scaling-crossshape.md`. |
